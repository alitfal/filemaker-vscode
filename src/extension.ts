...
import { FMConnectionManager } from './fmConnectionManager';
import { FMCompletionProvider } from './fmCompletionProvider';
...
const FM_DOCS: Record<string, { signature: string; description: string; example?: string }> = {
    ...
    'Ceiling': { signature: 'Ceiling ( number )', description: 'Returns number rounded up to the nearest integer.' },
    ...
    'JSONParse': { signature: 'JSONParse ( json )', description: 'Parses JSON text and caches the result in memory for reuse. FM 2025+' },
    ...
    'JSONSetElement': { signature: 'JSONSetElement ( json ; keyOrIndexOrPath ; value ; type )', description: 'Sets or adds an element in json.', example: 'JSONSetElement ( "{}" ; "name" ; "FileMaker" ; JSONString )' },
    'Case': { signature: 'Case ( test1 ; result1 {; test2 ; result2 ; ...} {; defaultResult} )', description: 'Returns the result corresponding to the first true test.' },
    ...
    'IsEmpty': { signature: 'IsEmpty ( field )', description: 'Returns True (1) if field contains no value.' },
    ...
    'Let': { signature: 'Let ( {[} var1 = expression1 {; var2 = expression2...{]}}} ; calculation )', description: 'Assigns values to variables and evaluates calculation.', example: 'Let ( [ x = 10 ; y = 20 ] ; x + y )' },
    ...
    'Average': { signature: 'Average ( field {; field...} )', description: 'Returns the average of all non-empty values.' },
    'Count': { signature: 'Count ( field {; field...} )', description: 'Returns the count of non-empty values.' },
    'Max': { signature: 'Max ( field {; field...} )', description: 'Returns the maximum value.' },
    'Min': { signature: 'Min ( field {; field...} )', description: 'Returns the minimum value.' },
    'Sum': { signature: 'Sum ( field {; field...} )', description: 'Returns the sum of all non-empty values.' },
    'StDev': { signature: 'StDev ( field {; field...} )', description: 'Returns the standard deviation of a sample.' },
    'Variance': { signature: 'Variance ( field {; field...} )', description: 'Returns the variance of a sample.' },
    'GetEmbedding': { signature: 'GetEmbedding ( modelName ; inputData )', description: 'Sends inputData to the embedding model and returns the vector. FM 2024+' },
    ...
    'GetTableDDL': { signature: 'GetTableDDL ( tableOccurrences )', description: 'Returns table schema in DDL format as a JSON array. FM 2025+' },
    ...
    'SubtractEmbeddings': { signature: 'SubtractEmbeddings ( embedding1 ; embedding2 )', description: 'Subtracts embedding2 from embedding1. FM 2024+' },
    ...
    'CryptGeneratePassKey': { signature: 'CryptGeneratePassKey ( passPhrase ; algorithm )', description: 'Generates a cryptographic key from passPhrase. FM 2023+' },
    ...
    'ExecuteSQL': { signature: 'ExecuteSQL ( sqlQuery ; fieldSeparator ; rowSeparator {; arguments...} )', description: 'Executes an SQL SELECT query against the FileMaker database.', example: 'ExecuteSQL ( "SELECT Name FROM Contacts WHERE Active = ?" ; "" ; "¶" ; 1 )' },
    ...
    'GetLiveTextAsJSON': { signature: 'GetLiveTextAsJSON ( field )', description: 'Returns text recognized from an image as JSON with position info. FM 2024+' },
    ...
    'GetRecordIDsFromFoundSet': { signature: 'GetRecordIDsFromFoundSet ( type )', description: 'Returns record IDs from the current found set. FM 2025+' },
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // ── Connection Manager ────────────────────────────────────────────────
    const connectionManager = new FMConnectionManager(context);

    // ── Completion Provider ───────────────────────────────────────────────
    const completionProvider = new FMCompletionProvider(() => connectionManager.getSchema());
    ...
    // ── Commands ──────────────────────────────────────────────────────────
    context.subscriptions.push(
        vscode.commands.registerCommand('functions.jsonToJsonSetElement', () => {
            ...
            try {
                ...
                editor.edit(editBuilder => editBuilder.replace(editor.selection, jsonSetElement));
            } catch (error: unknown) {
                vscode.window.showErrorMessage(error instanceof Error ? error.message : String(error));
            }
        }),

        vscode.commands.registerCommand('filemaker.connect', () => connectionManager.connect()),
        vscode.commands.registerCommand('filemaker.switchConnection', () => connectionManager.switchConnection()),
        vscode.commands.registerCommand('filemaker.refreshSchema', () => connectionManager.refreshSchema()),
        vscode.commands.registerCommand('filemaker.disconnect', () => connectionManager.disconnect()),
        vscode.commands.registerCommand('filemaker.showStatus', () => connectionManager.showStatus()),
    );
