...
const FM_DOCS: Record<string, { signature: string; description: string; example?: string }> = {
  ...
  'Lower': { signature: 'Lower ( text )', description: 'Returns text converted to lowercase.', example: 'Lower ( "FileMaker" ) // Returns "filemaker"' },
  ...
  'SortValues': { signature: 'SortValues ( textList ; sortType )', description: 'Returns a sorted list. sortType: 1=text ascending, 2=text descending, 3=numeric asc, 4=numeric desc.' },
  ...
  'JSONGetElementType': { signature: 'JSONGetElementType ( json ; keyOrIndexOrPath )', description: 'Returns the data type of the element: JSONString(1), JSONNumber(2), JSONBoolean(3), JSONArray(4), JSONObject(5), JSONNull(6), JSONRaw(7).' },
  ...
  const hoverProvider = vscode.languages.registerHoverProvider(
    { language: 'filemaker' },
    {
      provideHover(document, position) {
        ...
        // Link to docs
        const docsUrl = `https://help.claris.com/en/pro-help/content/functions-reference.html`;
        md.appendMarkdown(`\n[📖 FileMaker Documentation](${docsUrl})`);
